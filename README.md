...
REST API приложение, представляющее собой соцсеть.
...
```bash  
git clone https://github.com/emilversiontwo/laravel-filament.git
cd laravel-filament
```
...
- API: `http://localhost:8081/api` (при `APP_PORT=8081`).
- Swagger документация: `http://localhost:8081/swagger` (при `APP_PORT=8081`).
- Или воспользуйтесь: [laravel-filament.postman_collection.json](../laravel-filament.postman_collection.json)
- При использывании Swagger измените `APP_URL=http://127.0.0.1:<APP_PORT>`
- Filament Panel: http://localhost:8081/admin
- Emil: `admin@example.com`
- Password: `password`
- Для того чтобы получить доступ к Filament Panel, выполните:
```bash 
task dbfs
```  
или
```bash  
task artisan -- migrate && task artisan -- db:seed
```  
